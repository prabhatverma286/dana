...
########################################################################################################################
# The following code was checked out from https://github.com/AurelianTactics/dqfd-with-keras by author
...
from tensorflow.keras.layers import Dense, Dropout, Flatten, Input, Lambda, Conv2D
...
constants = {
    ...
    'demo_dir': 'sonic_demos/',
    'ebuff_save_name': 'expert_transitions',
    'out_dir': 'agent-training'
}


def fill_expert_buffer(env, exp_buffer, nsteps=10):
    ...
    if isdir(constants['demo_dir']):
        onlyfiles = [f for f in listdir(constants['demo_dir']) if isfile(join(constants['demo_dir'], f))]
        onlyfiles.sort()
        ...
        for file in onlyfiles:
            if demo_files_to_parse < 1:
                break
            if ".bk2" in file and constants['game'] in file and constants['state'] in file:
                exp_buffer = parse_demo(env, exp_buffer, "{}{}".format(constants['demo_dir'],file), nsteps)
                demo_files_to_parse -= 1
...
def make_env(game='SonicTheHedgehog-Genesis', state='GreenHillZone.Act1', stack=True, scale_rew=True, scenario = 'contest',
             is_contest_env=True, action_list=None, allow_backtracking=True):
    if action_list is None:
        action_list = [['LEFT'], ['RIGHT'], ['B']]
    if is_contest_env:
        env = make(game=game, state=state, bk2dir='videos')
...
def build_model(action_len, img_rows=84, img_cols=84, img_channels=4, dueling=True, clip_value=1.0,
                learning_rate=1e-4, dqn_loss=1.0, nstep_reg=1.0, slmc_reg=1.0, l2_reg=10e-5):
    ...
    model.compile(optimizer=adam,
                  loss=['mse', 'mse', 'mae'],
                  loss_weights=[dqn_loss, nstep_reg, slmc_reg])
...
def train_network(env, train_model, target_model, exp_buffer, rep_buffer, action_len,max_timesteps=1000000,min_buffer_size=20000,
                  epsilon_start = 0.99,epsilon_min=0.01,nsteps = 10, batch_size = 32, update_every=10000, expert_margin=0.8,
    ...
    while train_ts < max_timesteps:
        ...
        if _done:
            ...
            file_name = os.path.join(constants['out_dir'], str(env.episode_id) + '-meta.json')
            os.makedirs(os.path.dirname(file_name), exist_ok=True)
            with open(file_name, 'w+') as fd:
                fd.write(json.dumps({
                    'episode_number': env.episode_id,
                    'episode_score': total_rew
                }))
    ...
    zString = "dqfd_training_weights/SonicTheHedgehog-Genesis_dqfd_model.h5"
    train_model.save_weights(zString, overwrite=True)
    with open("sonic_episode_rewards.json") as f:
        json_dic = {'sonic': episode_rewards}
...
def train_expert_model(train_model, target_model, exp_buffer, action_len, batch_size=32,train_steps=750000,update_every=10000,
    ...
    zString = "dqfd_training_weights/expert_model_final.h5"
    train_model.save_weights(zString, overwrite=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--json_arguments', type=json.loads, help='arguments to run the script with', default=None)
    args = parser.parse_args()

    arguments = args.json_arguments
    ...
    if arguments['train_expert']:
        env, n_action = make_env(game=constants['game'], state=constants['state'], stack=True,
                                 scale_rew=arguments['reward_clipping'], scenario='contest', is_contest_env=False,
                                 action_list=action_list, allow_backtracking=arguments['allow_backtracking'])
        ...
        expert_buffer = replay.PrioritizedReplayBuffer(arguments["buffer_size"],
                                                       alpha=arguments['prioritized_replay_alpha'],
                                                       beta=arguments['prioritized_replay_beta'],
                                                       epsilon=arguments['prioritized_replay_eps'])
        expert_buffer = fill_expert_buffer(env, expert_buffer)

        temp_model = build_model(n_action, dueling=arguments['dueling'],
                                 learning_rate=arguments['lr'], dqn_loss=arguments['dqn_loss'],
                                 nstep_reg=arguments['nstep_loss'], slmc_reg=arguments['slmc_loss'],
                                 l2_reg=arguments['l2_loss'])
        tgt_model = build_model(n_action, dueling=arguments['dueling'],
                                 learning_rate=arguments['lr'], dqn_loss=arguments['dqn_loss'],
                                 nstep_reg=arguments['nstep_loss'], slmc_reg=arguments['slmc_loss'],
                                 l2_reg=arguments['l2_loss'])
        tgt_model, expert_buffer = train_expert_model(temp_model, tgt_model, expert_buffer, n_action,
                                                      update_every=arguments['target_network_update_freq'],
                                                      gamma=arguments['gamma'],
                                                      exp_margin_constant=arguments['demo_ratio'],
                                                      batch_size=arguments['batch_size'],
                                                      train_steps=arguments['total_timesteps_expert'])  # 10 #750000
        expert_buffer.save_samples(constants['ebuff_save_name'])

    else:
        print('Training DQfD model')
        env, n_action = make_env(game=constants['game'], state=constants['state'], stack=True, scale_rew=False, scenario='contest',
                                 action_list=action_list, allow_backtracking=arguments['allow_backtracking'])

        env = Monitor(env, directory=constants['out_dir'], force=True, video_callable=lambda x: True)
        ...
        expert_buffer = replay.PrioritizedReplayBuffer(arguments['buffer_size'],
                                                       ...
                                                       beta=arguments['prioritized_replay_beta'],
                                                       epsilon=arguments['prioritized_replay_eps'])
        expert_buffer.load_samples(constants['ebuff_save_name'])
        replay_buffer = replay.PrioritizedReplayBuffer(arguments['buffer_size'],
                                                       alpha=arguments['prioritized_replay_alpha'],
                                                       beta=arguments['prioritized_replay_beta'],
                                                       epsilon=arguments['prioritized_replay_eps'])
        # load trained model
        ...
        tgt_model.load_weights('dqfd_training_weights\\' + arguments['expert_model_name'])

        init_model = build_model(n_action, dueling=arguments['dueling'],
                                 learning_rate=arguments['lr'], dqn_loss=arguments['dqn_loss'],
                                 nstep_reg=arguments['nstep_loss'], slmc_reg=arguments['slmc_loss'],
                                 l2_reg=arguments['l2_loss'])

        init_model.load_weights('dqfd_training_weights\\' + arguments['expert_model_name'])

        # train model
        train_network(env, init_model, tgt_model,expert_buffer,replay_buffer, n_action,
                      min_buffer_size=arguments['learning_starts'],
                      epsilon_start=arguments['exploration_eps_start'],
                      epsilon_min=arguments['exploration_min_eps'],
                      batch_size=arguments['batch_size'],
                      update_every=arguments['target_network_update_freq'],
                      expert_margin=arguments['demo_ratio'],
                      gamma=arguments['gamma'],
                      max_timesteps=arguments['total_timesteps_normal'])  # min_buffer_size=50
